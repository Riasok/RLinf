...
OUT=$ROOT/logs/20260915-ppo-step136-spatial-ns5
mkdir -p "$OUT" /tmp/p136-ray /tmp/p136-tmp
rm -rf /tmp/p136-ray; mkdir -p /tmp/p136-ray
cd "$ROOT"
printf '%s START PPO step136 spatial ns5\n' "$(date '+%F %T %Z')" > "$OUT/state.log"
...
export RAY_TMPDIR=/tmp/p136-ray TMPDIR=/tmp/p136-tmp
export EVAL_GPU_RANGE=0-3
export EVAL_LOG_DIR=$OUT EVAL_NAME=ppo_step136_spatial_ns5
...
export EVAL_CKPT_PATH=$ROOT/checkpoints/hf-rlinf-ppo-spatial-step136/global_step_136/actor/model_state_dict/full_weights.pt
